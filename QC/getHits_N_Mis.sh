...
/share/apps/samtools/base/1.10/bin/samtools view $bam | awk '{for(i=1;i<=NF;i++){if($i~/NH:i:/){Hits[$i]++} if($i~/[n,N]M:i:/){Mis[$i]++}}}END{for(hit in Hits){print hit"\t"Hits[hit]/1000000 > "'$hits'.tmp"}; for(mis in Mis){print mis"\t"Mis[mis]/1000000 > "'$mis'.tmp"}}'
